...
namespace SFA.DAS.DfESignIn.Auth.UnitTests.Services.OidcService;

public class WhenPopulatingAccountClaims
{
    [Test, MoqAutoData]
    public async Task If_Token_TokenEndpointPrincipal_Is_Null_Then_Not_Updated(DfESignInUser user, IOptions<DfEOidcConfiguration> config, string accessToken)
    {
        //Arrange
    ...
    [Test, MoqAutoData]
    public async Task If_Token_TokenEndpointResponse_Is_Null_Then_Not_Updated(DfESignInUser user, IOptions<DfEOidcConfiguration> config)
    {
        //Arrange
        config.Value.BaseUrl = $"https://{config.Value.BaseUrl}";
        var response = new HttpResponseMessage
    ...
    public async Task Then_The_User_Endpoint_Is_Called_Using_AccessToken_From_TokenValidatedContext(
        ...
        IOptions<DfEOidcConfiguration> config)
    {
        //Arrange
        config.Value.BaseUrl = $"https://{config.Value.BaseUrl}";
        var mockPrincipal = new Mock<ClaimsPrincipal>();
        ...
        IOptions<DfEOidcConfiguration> config)
    {
        ...
        var response = new HttpResponseMessage
        {
            Content = new StringContent(JsonSerializer.Serialize((DfESignInUser)null!)),
            StatusCode = HttpStatusCode.Accepted
        };
        var expectedUrl = new Uri($"{config.Value.BaseUrl}/userinfo");
        var httpMessageHandler = MessageHandler.SetupMessageHandlerMock(response, expectedUrl, HttpMethod.Get);
