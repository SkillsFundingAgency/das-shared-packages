...
public class WhenCallingHeadResponseCode
{
    [Test, AutoData]
        ...
        TestInternalApiConfiguration config)
    {
        ...
        var headTestRequest = new HeadTestRequest(id);
        var expectedUrl = $"{config.Url}{headTestRequest.HeadUrl}";
        var httpMessageHandler = MessageHandler.SetupMessageHandlerMock(response, expectedUrl, "head");
        ...
        httpMessageHandler.Protected()
            .Verify<Task<HttpResponseMessage>>(
                ...
                ItExpr.Is<HttpRequestMessage>(c =>
                    c.Method.Equals(HttpMethod.Head)
    ...
    private class HeadTestRequest : IHeadApiRequest
    {
        private readonly int _id;

        public string Version => "2.0";

        public HeadTestRequest(int id)
        {
            _id = id;
        }

        public string HeadUrl => $"/test-url/head{_id}";
    }
}
