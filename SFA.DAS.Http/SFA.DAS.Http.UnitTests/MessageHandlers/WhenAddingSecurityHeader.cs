...
{
    [TestFixture]
    public class WhenAddingSecurityHeader
    {
        [Test]
        public async Task ThenBearerHeaderShouldBeAdded()
        {
            var mockTokenGenerator = new Mock<IGenerateBearerToken>();
            mockTokenGenerator.Setup(x => x.Generate()).ReturnsAsync("TestTokenValue");

            var handler = new SecurityMessageHandler(mockTokenGenerator.Object) { InnerHandler = new StubResponseHandler() };
